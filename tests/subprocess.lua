...
function test_callback()
	...
	--[[
	Note: We intentionally use here stderr because for some reason if we use
	stdout a message about test environment is printed.
	]]
	local ok, out = subprocess(LST_HOOK, "Test: env", 1000, function () io.stderr:write("Hello callback") end, "true")
	assert_equal(0, ok)
	assert_equal("Hello callback", out)
